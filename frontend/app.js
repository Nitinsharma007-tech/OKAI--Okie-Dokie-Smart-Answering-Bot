...
(function initUIEnhancements() {
    ...
    // 3. Magnetic buttons (skip on touch/no-hover devices) — no visible cursor dot, just the pull
    if (window.matchMedia && window.matchMedia("(hover: hover) and (pointer: fine)").matches) {
        let mouseX = window.innerWidth / 2, mouseY = window.innerHeight / 2;
        const MAGNET_SELECTOR = ".send-btn, .chat-fab, .icon-btn, .chat-close-btn";
        const MAGNET_RADIUS = 75;
        const MAGNET_STRENGTH = 0.35;
        let magneticEls = [];

        function refreshMagneticEls() {
            magneticEls = Array.from(document.querySelectorAll(MAGNET_SELECTOR));
            magneticEls.forEach(el => {
                if (!el.dataset.magnetReady) {
        ...
        refreshMagneticEls();
        setInterval(refreshMagneticEls, 1200);
    ...
    // 4. Magic Bento Cards — spotlight, border glow, sparks, click burst, magnetism
    const BENTO_SELECTOR = ".stat-pill, .ai-chip, .welcome-step, .tree-module-header, .doc-card, .glass-card, .ask-ai-btn";
    const BENTO_GLOW = "255, 122, 41"; // orange
    const BENTO_RADIUS = 400;          // spotlightRadius
    const BENTO_PARTICLE_GAP_MS = 90;  // paced to roughly match particleCount≈12 over a hover
    const BENTO_MAGNETISM = true;
    const BENTO_MAGNET_STRENGTH = 0.06;

    function spawnSpark(el, x, y) {
        const spark = document.createElement("span");
        spark.className = "bento-spark";
    ...
    function setupBentoCard(el) {
        if (el.dataset.bentoReady) return;
        el.dataset.bentoReady = "true";
        ...
        el.addEventListener("mousemove", (e) => {
            ...
            const intensity = Math.max(0, 1 - dist / BENTO_RADIUS);

            el.style.setProperty("--mx", `${x}px`);
            el.style.setProperty("--my", `${y}px`);
            el.style.setProperty("--glow-o", intensity.toFixed(2));
            el.classList.toggle("bento-active", intensity > 0.05);

            if (BENTO_MAGNETISM) {
                el.style.transform = `translate(${(x - cx) * BENTO_MAGNET_STRENGTH}px, ${(y - cy) * BENTO_MAGNET_STRENGTH}px)`;
            }

            const now = performance.now();
            if (now - lastSpark > BENTO_PARTICLE_GAP_MS) {
                lastSpark = now;
                spawnSpark(el, x, y);
            }
        });
        ...
        el.addEventListener("mouseleave", () => {
            el.classList.remove("bento-active");
        ...
        el.addEventListener("click", (e) => {
            const rect = el.getBoundingClientRect();
            spawnBurst(el, e.clientX - rect.left, e.clientY - rect.top);
        });
    }

    function initBentoCards() {
        document.querySelectorAll(BENTO_SELECTOR).forEach(setupBentoCard);
    }
    initBentoCards();
    setInterval(initBentoCards, 1200); // picks up cards rendered after initial load (tree modules, doc cards, etc.)
})();